  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 w-8 h-8 rounded-full bg-white/90 shadow border border-slate-100 hover:bg-slate-50 flex items-center justify-center text-slate-500 transition-colors"
        >
          <X className="w-4 h-4" />
        </button>

        {/* Main content */}
        <div className="flex flex-col sm:flex-row" style={{ minHeight: 420, maxHeight: "calc(90vh - 56px)" }}>

          {/* Left — photo with pastel bg, LinkedIn badge bottom-right */}
          <div
            className={`relative flex-shrink-0 ${member.pastel} overflow-hidden`}
          ...
          >
            ...
            <a
              ...
              onClick={(e) => e.stopPropagation()}
              className="absolute bottom-4 right-4 w-10 h-10 bg-black hover:bg-[#0077B5] rounded-xl flex items-center justify-center transition-colors shadow-lg"
            >
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <h3 className="text-3xl font-bold font-display text-[#0c1e24] mb-1 leading-tight">
              {member.name}
            </h3>
            <p className="text-slate-500 text-base mb-5">{member.position}, {member.company}</p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-100">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-xs font-semibold bg-[#25D366] hover:bg-[#1ebe5c] text-white px-4 py-2 rounded-full transition-colors"
              >
        ...
        {/* Carousel nav */}
        <div className="px-7 py-3.5 border-t border-slate-100 flex items-center justify-between">
