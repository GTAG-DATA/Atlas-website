    ...
    >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row" style={{ maxHeight: "calc(90vh - 64px)" }}>
          {/* Photo — fixed 220×220 square, self-start so pastel bg never bleeds below image */}
          <div
            className={`flex-shrink-0 self-start ${member.pastel} overflow-hidden`}
            style={{ width: 220, minWidth: 220, height: 220 }}
          >
            <img
              src={member.image}
              alt={member.name}
          ...
          <div className="flex-1 p-7 overflow-y-auto">
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 space-y-2.5">
              ...
              {/* Row 2: LinkedIn (blue pill) + Email */}
              <div className="flex flex-wrap items-center gap-3">
                <a href={member.linkedin} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 text-xs font-semibold bg-[#0077B5] hover:bg-[#006097] text-white px-3 py-1.5 rounded-full transition-colors">
                  <Linkedin className="w-3.5 h-3.5" /> LinkedIn
                </a>
                <a href={`mailto:${member.email}`}
                  className="text-xs font-semibold text-slate-500 hover:text-[#0c1e24] transition-colors">
                  {member.email}
                </a>
              </div>
            </div>
...
// ── Partners Showcase ─────────────────────────────────────────
function PartnersShowcase({ onSelect }: { onSelect: (index: number) => void }) {
  const [hovered, setHovered] = useState<number | null>(null);
  const partners = teamData.slice(1); // Peter, David, Alexia
  const topOffsets = [0, 72, 36]; // stagger vertical offset per column (px)

  return (
    <div className="flex flex-col lg:flex-row gap-10 xl:gap-16 items-start">
      {/* Staggered photo grid */}
      <div className="flex gap-4 sm:gap-5 flex-shrink-0" style={{ paddingBottom: 72 }}>
        {partners.map((member, i) => (
          <div
            key={member.name}
            className="cursor-pointer group"
            style={{ marginTop: topOffsets[i], width: 155 }}
            onClick={() => onSelect(i + 1)}
            onMouseEnter={() => setHovered(i)}
            onMouseLeave={() => setHovered(null)}
          >
            <div className={`overflow-hidden rounded-2xl ${member.pastel}`} style={{ height: 205 }}>
              <img
                src={member.image}
                alt={member.name}
                className={`w-full h-full object-cover object-top transition-all duration-500 group-hover:scale-105 ${
                  hovered !== null && hovered !== i ? "grayscale opacity-60" : ""
                }`}
              />
            </div>
            <p className="text-xs font-semibold text-[#0c1e24] mt-2.5 text-center leading-tight">
              {member.name.split(",")[0]}
            </p>
            <p className="text-[10px] text-slate-400 text-center">{member.position}</p>
          </div>
        ))}
      </div>
          ...
          {partners.map((member, i) => (
            <div
              ...
              className={`flex items-center gap-4 p-4 rounded-xl cursor-pointer transition-all duration-200 ${
                hovered === i ? "bg-[#142E36]/5" : "hover:bg-slate-50"
              }`}
              onMouseEnter={() => setHovered(i)}
              onMouseLeave={() => setHovered(null)}
              onClick={() => onSelect(i + 1)}
            >
              ...
              <div
                className={`w-1 rounded-full flex-shrink-0 transition-all duration-300 ${
                  hovered === i ? "h-10 bg-[#142E36]" : "h-5 bg-slate-200"
                }`}
              />
              {/* Name + role */}
              <div className="flex-1 min-w-0">
                <p className={`font-bold font-display text-sm transition-colors ${hovered === i ? "text-[#0c1e24]" : "text-slate-500"}`}>
                  {member.name}
                </p>
                <p className="text-xs text-slate-400">{member.position} · {member.company}</p>
              </div>
              {/* Slide-in actions */}
              <div
              ...
              >
                <a
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="w-7 h-7 rounded-full bg-[#25D366] text-white flex items-center justify-center hover:bg-[#1ebe5c] transition-colors"
                  title="WhatsApp"
                >
                ...
                <span className="text-xs text-amber-600 font-semibold whitespace-nowrap">View profile →</span>
              </div>
            </div>
          ))}
        </div>
        <p className="text-xs text-slate-400 mt-6 pl-5">Click any profile to view full bio &amp; credentials</p>
      </div>
    </div>
  );
}
        ...
        <div className="bg-slate-50 py-16 mb-20">
          <div className="max-w-5xl mx-auto px-6">
            ...
            <p className="text-slate-500 text-sm mb-10 max-w-lg">
              Hover to explore our partners — click any profile to view full credentials.
            </p>
            <PartnersShowcase onSelect={(i) => setModalIndex(i)} />
          </div>
        </div>
